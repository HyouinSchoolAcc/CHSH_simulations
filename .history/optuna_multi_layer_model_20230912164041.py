...
def objective(trial):
    # Define constants
    INPUT_DIM = 2  # Replace with your input dimension
    OUTPUT_DIM = 1  # Replace with your output dimension
    EPOCHS = trial.suggest_int("epochs", 10, 50)
    LR = trial.suggest_float("lr", 1e-5, 1e-1, log=True)

    n_layers = trial.suggest_int("n_layers", 1, 3)
    hidden_dims = [trial.suggest_int(f"n_units_l{i}", 10, 100) for i in range(3)]
    hidden_layers = hidden_dims[:n_layers]

    model = DynamicNN(input_dim=INPUT_DIM, output_dim=OUTPUT_DIM, hidden_layers=hidden_layers)
    model = model.float()  # Ensure model is in Float type
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            output = output
            target = target.float().reshape(output.shape)  # Adjust the shape to match the output shape
