...
# Suggest number of neurons for each layer with conditional suggestion
    hidden_layers = []
    for i in range(3):  # We use a fixed range up to max number of layers
        if i < n_layers:
            hidden_dim = trial.suggest_int(f"n_units_l{i}", 10, 100)
            hidden_layers.append(hidden_dim)
        else:
            trial._suggest(f"n_units_l{i}", 0)  # This suggests a fixed value (0), ensuring consistency across trials
