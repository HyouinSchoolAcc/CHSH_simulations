...
class MyCNN(nn.Module):
    def __init__(self, num_conv_layers, num_fc_layers, num_kernels, fc_units):
        ...
        input_channel = 2  # Changing this to reflect a 4x4x2 input
        ...
        input_size = input_channel * 4 * 2  # Assuming a size of 4x4x2 matrix after convolution and pooling
        print(f"Expected flattened size before FC layers: {input_size}")
    ...
    def forward(self, x):
        x = x.permute(0, 3, 1, 2)  # Permuting the dimensions to be [batch_size, channels, height, width]

        for conv, relu, pool in zip(self.convs, self.relus, self.pools):
            x = pool(relu(conv(x.float())))
            print(f"After Conv + ReLU + Pool tensor shape: {x.shape}")
        
        print(f"Tensor shape before flattening: {x.shape}")
        x = x.view(x.size(0), -1)  # Flatten
        print(f"Tensor shape after flattening: {x.shape}")

        for fc in self.fcs[:-1]:
            print(f"FC Layer input shape: {x.shape} and FC layer expected input size: {fc.in_features}")
            x = torch.relu(fc(x))
        x = self.fcs[-1](x)
