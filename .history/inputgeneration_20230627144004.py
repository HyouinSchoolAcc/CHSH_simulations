...
def generate_psd_matrix_with_eigenvalues():
    ...
    hermitian_matrix1 = np.dot(eigenvectors1/sum(eigenvectors1), np.diag(eigenvalues1))
    matrix1 = np.dot(hermitian_matrix1, np.transpose(np.conjugate(eigenvectors1)/sum(eigenvectors1)))
...
psd_matrices = [generate_psd_matrix_with_eigenvalues() for a in 30]
