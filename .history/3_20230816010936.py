...
class MyCNN(nn.Module):
    def __init__(self, num_conv_layers, num_fc_layers, num_kernels, fc_units):
        ...
        input_channel = 2  # Starting with 2 channels in input
        output_height, output_width = 4, 4
        
        for i in range(num_conv_layers):
            self.convs.append(nn.Conv2d(input_channel, num_kernels[i], kernel_size=3, stride=1, padding=1))
            self.relus.append(nn.ReLU())
            
            # Make sure we're not reducing the width too much
            if output_width > 2:
                self.pools.append(nn.MaxPool2d(kernel_size=(1, 2), stride=(1, 2))) 
                output_width //= 2
            else:
                self.pools.append(nn.MaxPool2d(kernel_size=1, stride=1))  # Essentially doing nothing
                
            input_channel = num_kernels[i]
        
        self.fcs = nn.ModuleList()
        input_size = input_channel * output_height * output_width
