...
def rank_data(data):
    # Get the rank of the elements
    temp = data.argsort()
    ranks = np.empty_like(temp)
    ranks[temp] = np.arange(len(data))
    return ranks + 1  # +1 to make the rank start from 1 instead of 0
...
Spearman_correlation_transfer_frozen = spearman_rank_correlation(all_predictions, labels_flat) # Assuming y_test is the true test labels
