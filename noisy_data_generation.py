...
def function_2_noisy(A_1,B_1,matrix):
    A = np.deg2rad(A_1)
    B = np.deg2rad(B_1)
    ...
    return (result_vector1+random.uniform(-0.2, 0.2),result_vector2+random.uniform(-0.2, 0.2),result_vector3+random.uniform(-0.2, 0.2),result_vector4+random.uniform(-0.2, 0.2))
...
#now we will have 3 noises generated, this first one will be the angle uncertainty noise, where the measurement angles are += 5 degrees
noise = [1,0,0]
for a in range (5000):
    matrix = generate_psd_matrix_with_eigenvalues()
    CHSH_result = CHSH_val_calc(matrix, T_11, T_12, T_13, T_21, T_22, T_23, T_31, T_32, T_33)
    result.append(function_2(0+random.uniform(0, 5), 22.5+random.uniform(0, 5), matrix))
    result.append(function_2(0+random.uniform(0, 5), 67.5+random.uniform(0, 5), matrix))
    result.append(function_2(45+random.uniform(0, 5), 22.5+random.uniform(0, 5), matrix))
    result.append(function_2(0+random.uniform(0, 5), 67.5+random.uniform(0, 5), matrix))
    result.append(function_2(302.6487+random.uniform(0, 5), 80.4775+random.uniform(0, 5), matrix))
    result.append(function_2(199.3459+random.uniform(0, 5), 307.7366+random.uniform(0, 5), matrix))
    result.append(function_2(221.461+random.uniform(0, 5), 99.953+random.uniform(0, 5), matrix))
    result.append(function_2(13.686+random.uniform(0, 5),140.040+random.uniform(0, 5), matrix))
    results.append(np.array(result).flatten()) #results should now have a 32-element array, this should be written to the file and the output be the CHSH value.
    result.clear()
    ...
    with open(file_path, "a") as file:
        for res in results:
            ...
            file.write("; " + str(noise[0]) + ", " + str(noise[1]) + ", " + str(noise[2]) + "\n")
    #         print(res)
...
#this second one will be the shot noise uncertainty, where the measurement results are +- 0.2.
noise = [0,1,0]
for a in range (5000):
    matrix = generate_psd_matrix_with_eigenvalues()
    CHSH_result = CHSH_val_calc(matrix, T_11, T_12, T_13, T_21, T_22, T_23, T_31, T_32, T_33)
    result.append(function_2_noisy(0, 22.5, matrix))
    result.append(function_2_noisy(0, 67.5, matrix))
    result.append(function_2_noisy(45, 22.5, matrix))
    result.append(function_2_noisy(0, 67.5, matrix))
    result.append(function_2_noisy(302.6487, 80.4775, matrix))
    result.append(function_2_noisy(199.3459, 307.7366, matrix))
    result.append(function_2_noisy(221.461, 99.953, matrix))
    result.append(function_2_noisy(13.686,140.040, matrix))
    results.append(np.array(result).flatten()) #results should now have a 32-element array, this should be written to the file and the output be the CHSH value.
    result.clear()
    #write the results to a file
    #now we will add in the results value to the file, and then the CHSH value, alternating
...
#this third one will be the phase noise uncertainty, where a random phase will be applied to the density matrix.
noise = [0,0,1]
def apply_unitary(density_matrix, unitary):
    # Apply the unitary transformation
    return unitary @ density_matrix @ unitary.conjugate().T

# Define the phase angles for each qubit
theta1 = np.random.uniform(0, 2*np.pi)
theta2 = np.random.uniform(0, 2*np.pi)

# Define the unitary matrices for each qubit
U1 = np.array([[1, 0], [0, np.exp(1j*theta1)]])
U2 = np.array([[1, 0], [0, np.exp(1j*theta2)]])

# Combined unitary matrix for the two-qubit system
U_combined = np.kron(U1, U2)


for a in range (5000):
    matrix1 = generate_psd_matrix_with_eigenvalues()
    # Apply the unitary transformation
    matrix = apply_unitary(matrix1, U_combined)
    CHSH_result = CHSH_val_calc(matrix, T_11, T_12, T_13, T_21, T_22, T_23, T_31, T_32, T_33)
    result.append(function_2_noisy(0, 22.5, matrix))
    result.append(function_2_noisy(0, 67.5, matrix))
    result.append(function_2_noisy(45, 22.5, matrix))
